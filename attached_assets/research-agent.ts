...
import { performSearch, performFinancialSearch, extractWebContent } from '../tools/searchTool';
...
...`;
    ...
    const agreements = agreementsMatch && agreementsMatch[1]
      ...
      : ['No clear agreements found between sources.'];
    
    // Extract discrepancies (simplified parsing)
    const discrepancies = [];
    
    if (discrepanciesMatch && discrepanciesMatch[1]) {
      const discrepancyText = discrepanciesMatch[1];
      const discrepancyItems = discrepancyText.split(/\d+\.\s+/).slice(1);
      
      for (const item of discrepancyItems) {
        // Extract the discrepancy point (first line)
        const pointMatch = item.match(/^([^\n]+)/);
        const point = pointMatch ? pointMatch[1].trim() : 'Undefined discrepancy';
        
        // Extract different views
        const viewMatches = item.matchAll(/Source.+?view:([^\n]+)/gi);
        const views = Array.from(viewMatches).map(match => ({
          source: match[0].split(':')[0].trim(),
          view: match[1].trim()
        }));
        
        // Extract recommended interpretation
        const recMatch = item.match(/Recommended interpretation:([^\n]+)/i);
        const recommendation = recMatch ? recMatch[1].trim() : 'No specific recommendation provided.';
        
        discrepancies.push({
          point,
          different_views: views,
